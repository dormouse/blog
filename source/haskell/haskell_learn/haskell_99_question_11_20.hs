-- learn haskell 99 questions 11-20
-- website: https://wiki.haskell.org/99_questions/11_to_20
...
decodeModified [Multiple n x] = x : decodeModified [Multiple (n-1) x]
...
slice :: [a] -> Int -> Int -> [a]
slice xs n m = myTake (myDrop xs (n-1)) (m-n+1)
...
rotate :: [a] -> Int -> [a]
rotate xs n
  | n > 0 = snd(split xs n) ++ fst(split xs n)
  | n < 0 = snd(split xs (length xs + n)) ++ fst(split xs (length xs + n))
